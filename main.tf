...
resource "oci_core_instance" "homedock_worker" {
  count = var.num_worker_instances

  display_name        = "homedock-worker-${count.index + 1}-${random_string.resource_code.result}"
  compartment_id      = var.compartment_id
  ...
  metadata = {
    ssh_authorized_keys = local.instance_config.ssh_authorized_keys
    user_data           = base64encode(file("./bin/homedock-worker.sh"))
  }
